...
const NetworkDiagram = ({ nodeData, edgeData, width, height }) => {
  ...
  useEffect(() => {
    ...
    const links = edgeData.data.map((d) => ({
      source: d.node[0],
      target: d.node[1],
      type: d.type,
    }));
    ...
    linkHandler
      ...
      .attr("xlink:href", (d) => {
        ...
        if (d.type === Device.DistributionSwitch)
  ...
  const downloadSvg = () => {
    ...
    // Add XML namespace attributes if they are missing (important for proper rendering in some viewers)
    if (
    ...
    downloadLink.download = "d3_chart.svg"; // Default filename
